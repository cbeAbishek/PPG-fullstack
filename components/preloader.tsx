...
import { 
  Cpu, Cog, Heart, Activity, Leaf, Laptop, 
  Wifi, Database, PenTool, Thermometer, Droplet, 
  Layers, GitBranch 
} from 'lucide-react';
      ...
      <div className="text-center w-full max-w-md px-6">
        ...
        <div className="flex items-center justify-center mb-8">
          <div className="relative">
            <div className="w-32 h-32 rounded-full bg-gradient-to-br from-orange-400 to-orange-600 flex items-center justify-center">
              <div className="w-28 h-28 rounded-full bg-white flex items-center justify-center">
                <div className="w-24 h-24 rounded-full bg-gradient-to-tr from-orange-400 to-orange-600 flex items-center justify-center relative overflow-hidden">
                  ...
                  {/* Core elements */}
                  <div className="relative" style={{ transform: `rotate(${rotation/3}deg)` }}>
                    <Cog size={24} className="absolute text-white opacity-80" style={{ top: -20, left: -6 }} />
                    <Laptop size={28} className="text-white" />
                    <GitBranch size={16} className="absolute text-white opacity-80" style={{ bottom: -16, right: -10 }} />
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* Orbiting elements */}
            <div className="absolute top-0 left-0 w-full h-full animate-spin" style={{ animationDuration: '12s' }}>
            ...
            <div className="absolute top-0 left-0 w-full h-full animate-spin" style={{ animationDuration: '18s', animationDirection: 'reverse' }}>
              <div className="absolute" style={{ bottom: '30%', left: '10%' }}>
                <Wifi size={16} className="text-purple-500" />
              </div>
            </div>
            
            <div className="absolute top-0 left-0 w-full h-full animate-spin" style={{ animationDuration: '25s' }}>
        ...
        <div className="mb-6 h-20">
          <div className="flex items-center justify-center space-x-3 mb-2">
            ...
            <h2 className="text-xl font-bold text-gray-800">{departments[currentDept].name} Engineering</h2>
          </div>
          <p className="text-gray-500">Loading department resources...</p>
          <p className="text-gray-500">This is a Demo site infomation will not correct</p>
        </div>
        ...
        <p className="mt-2 text-gray-600">{Math.min(Math.floor(loadingProgress), 100)}%</p>
        
        {/* Engineering disciplines background */}
        <div className="mt-8 flex flex-wrap justify-center gap-3 opacity-60">
          {[Cog, Heart, Activity, Leaf, Laptop, Wifi, Database, PenTool, Thermometer, Droplet, Layers].map((Icon, idx) => (
            <div key={idx} className="p-2 animate-pulse" style={{ animationDelay: `${idx * 0.2}s` }}>
